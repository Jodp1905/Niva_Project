...
def plot_epoch_data(epoch_data_path, output_path, fold_number):
    ...
    metrics = [col for col in data.columns if col not in [
        'epoch', 'epoch_duration', 'rss_memory_gb', 'vms_memory_gb'] and not col.startswith('val_')]
    ...
    for i, metric in enumerate(metrics):
        ...
        plt.title(
            f'Fold {fold_number}: {metric.capitalize()} vs Validation {metric.capitalize()}')
        plt.xlabel('Epoch')
        plt.ylabel(metric)
        plt.legend()
...
def save_ploted_training_data(folder_path):
    ...
    fold_paths = [os.path.join(folder_path, f) for f in os.listdir(
        folder_path) if f.startswith('fold')]
    ...
    for fold_path in fold_paths:
        ...
        epoch_output_path = os.path.join(
            images_folder, f'fold_{fold_number}_epoch_metrics.png')
        batch_output_path = os.path.join(
            images_folder, f'fold_{fold_number}_batch_metrics.png')
...
def save_all_information_to_txt(folder_path, output_txt_path):
    with open(output_txt_path, 'w') as txt_file:
        ...
        hyperparameters_file = os.path.join(
            folder_path, 'hyperparameters.json')
        ...
        for fold_path in fold_paths:
            ...
            if os.path.exists(evaluation_file):
                ...
                txt_file.write(
                    f"Evaluation Data (Fold {fold_number}):\n{json.dumps(evaluation, indent=4)}\n\n")
            ...
            if os.path.exists(fold_info_file):
                ...
                txt_file.write(
                    f"Fold Info Data (Fold {fold_number}):\n{json.dumps(fold_info, indent=4)}\n\n")

        # Information from the "average" model
        avg_model_folder = os.path.join(
            folder_path, 'resunet-a_avg_2024-08-06-05-11-34+02-00')
        if os.path.exists(avg_model_folder):
            evaluation_avg_file = os.path.join(
                avg_model_folder, 'evaluation_avg.json')
            if os.path.exists(evaluation_avg_file):
                with open(evaluation_avg_file, 'r') as file:
                    avg_evaluation_data = json.load(file)
                txt_file.write(
                    f"Average Model Evaluation Data:\n{json.dumps(avg_evaluation_data, indent=4)}\n\n")


if __name__ == '__main__':
    # Creating arguments parser
    parser = argparse.ArgumentParser(
        description="Process CSV, txt and json files in a folder, save plots, and generate a summary report.")

    # Add arguments needed
    parser.add_argument(
        'folder_name', help="Path to the folder containing CSV files for each fold.")
    ...
    output_txt_path = os.path.join(
        folder_path, f"{folder_name}_experience_analyze.txt")
