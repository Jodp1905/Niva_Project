...
from pathlib import Path # Ajout joseph
...
def create_batch_request(batch: SentinelHubBatch,
    ...
    LOGGER.info('Set up SH credentials')
    ...
    output_path = Path(config.local_storage_path) / config.tiles_path / "<tileName>" / "<outputId>.<format>" #modif joseph
    output_path.parent.mkdir(parents=True, exist_ok=True)                                                    #modif joseph
    ...
    batch_request = batch.create(
        ...
        output=SentinelHubBatch.output(
            default_tile_path=str(output_path), #modif joseph
            skip_existing=False,
            overwrite=True
        ),
